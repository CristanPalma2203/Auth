...
namespace Dominio.Especificaciones
{
    public class BuscarUsuarioPorIdentificadorYContrasena : ISpecification<Usuario>
    {
        ...
        public Func<Usuario, bool> Traer()
        {
            ...
            {
                return new Func<Usuario, bool>(c => c.AccessIdentifier.ToLower().Trim() == this.identificador.ToLower().Trim() && c.Password == pass);
            }
            else {
                return new Func<Usuario, bool>(c => c.AccessIdentifier.Replace("-", "").Trim() == this.identificador.Replace("-", "").Trim() && c.Password == pass);
            }
